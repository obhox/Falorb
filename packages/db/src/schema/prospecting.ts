...
/**
 * One connected external service — Clay only for now. First real instance of
 * the integrations layer FEATURES.md §13 designed but never built; adapted
 * from that section's proposed shape. `integration_mappings` (arbitrary
 * inbound/outbound field mapping) is deliberately not built yet — Clay's
 * request/response shape is fixed and controlled on both ends, so there is
 * nothing to map until a second integration needs configurable mapping.
 */
export const integrations = pgTable(
  "integrations",
  {
    ...
    organizationId: uuid("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "cascade" }),
    /** "clay" today. Open text, same reasoning as `prospects.source`. */
    kind: text("kind").notNull().default("clay"),
    /** "disconnected" | "connected" | "error" */
    status: text("status").notNull().default("disconnected"),
    /** Non-secret settings only — e.g. which Clay table/workbook to target. */
    config: jsonb("config").notNull().default({}),
    /** Envelope-encrypted API key — see `crypto.ts`. Null until connected.
     * Never selected back to a client; only decrypted inside the worker at
     * call time. */
    encryptedCredential: text("encrypted_credential"),
    /** Non-secret preview (e.g. last 4 chars) so the UI can show *which* key
     * is stored, mirrors `apiKeys.keyPrefix`'s reasoning. */
...
/** Run history for one integration's sync — makes a silently broken
 * connection visible instead of looking like "no new prospects enriched". */
